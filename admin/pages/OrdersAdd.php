                    ...
                    <div class="col-md-6 col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Sipariş Oluşturma</h4>
                            </div>
                            <div class="card-body">
                                <form id="orderEkleForm" class="form form-horizontal">
                                    <div class="row">
                                    ...
                                    <?php if($settings['stockSystem']=='1'){ ?>
                                        <div class="col-12">
                                            <div class="mb-1 row">
                                                ...
                                                <div class="col-sm-9">
                                                <select class="js-example-basic-multiple" id="urunler" name="urunler[]" multiple="multiple">
                                                   <?php $urunler = $DB->query("SELECT * from products order by id desc")->fetchAll(PDO::FETCH_ASSOC); 
                                                    foreach ($urunler as $urunlerCek) {
                                                        ?>
                                                  <option <?php if($urunlerCek['stok']=='0'){echo 'disabled';} ?> value="<?=$urunlerCek['id']?>"><?=$urunlerCek['ad']?> | <?=$urunlerCek['fiyat']?>₺ | <?php if($urunlerCek['stok']=='0'){echo 'Stok Yok';} ?> <?php if($urunlerCek['stok']>'0'){echo 'Stok: '.$urunlerCek['stok'];} ?></option>
                                                  <?php } ?>
                                                </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="mb-1 row">
                                                ...
                                                <select class="js-example-basic-multiple" id="urunler" name="urunler[]" multiple="multiple">
                                                   <?php $urunler = $DB->query("SELECT * from products order by id desc")->fetchAll(PDO::FETCH_ASSOC); 
                                                    foreach ($urunler as $urunlerCek) {
                                                        ?>
                                                  <option value="<?=$urunlerCek['id']?>"><?=$urunlerCek['ad']?> | <?=$urunlerCek['fiyat']?>₺</option>
                                                  <?php } ?>
                                                </select>
                                                </div>
                                            </div>
                                      ...
                                      <div class="col-12">
                                          <div class="mb-1 row">
                                               <div class="col-sm-3">
                                                ...
                                                <div class="col-sm-9">  
                                                 <form id="tipForm">
                                                <input type="radio" id="mevcut" name="musteriTipi" value="Mevcut Müşteri" checked>
                                                ...
                                                <select class="js-example-basic-single" name="musteri">
                                                   <?php $musteriler = $DB->query("SELECT * from members order by id desc")->fetchAll(PDO::FETCH_ASSOC); 
                                                    foreach ($musteriler as $musterilerCek) {
                                                        ?>
                                                  <option value="<?=$musterilerCek['id']?>"><?=$musterilerCek['name']?></option>
                                                  <?php } ?>
                                                </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="col-12">
                                            <div class="mb-1 row">
                                                <div class="col-sm-3">
                                                    <label class="col-form-label" for="orderDate">Tarih</label>
                                                </div>
                                                <div class="col-sm-9">
                                                    <input type="datetime-local" id="orderDate" class="form-control"
                                                        name="orderDate" placeholder="Tarih" value="<?php echo date('Y-m-d\TH:i',time()) ?>"/>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-12">
                                            <div class="mb-1 row">
                                                ...
                                                foreach ($orderProvider as $orderProviderCek) {
                                                    ?>
                                                  <option value="<?=$orderProviderCek['id']?>"><?=$orderProviderCek['name']?></option>
                                                <?php } ?>
                                        ...
                                        <div class="col-sm-9 offset-sm-3">
                                            <input type="hidden" name="orderEkle" value="1">
                                            <button id="orderEkle" type="button"
